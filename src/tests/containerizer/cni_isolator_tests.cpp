        ...
        R"~(
          ...
          if [ "$CNI_COMMAND" = "ADD" ]; then
            for idx in `seq 0 127`; do
              ...
              ip netns exec $NETNS \
                  ip addr add "${VETH1_IP}/31" dev $VETH1
              ip netns exec $NETNS \
                  ip link set $VETH1 name $CNI_IFNAME up
              ip netns exec $NETNS \
                  ip route add default via $VETH0_IP dev $CNI_IFNAME
              break
            done
          else
            VETH1_IP=$(
              ip netns exec $NETNS ip addr show dev $CNI_IFNAME 2> /dev/null \
              | awk '/inet/{print $2}'
            )
          fi
...
TEST_F(CniIsolatorTest, ROOT_VerifyResourceStatistics)
{
  Try<Owned<cluster::Master>> master = StartMaster();
  ASSERT_SOME(master);
